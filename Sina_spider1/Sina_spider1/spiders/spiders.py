...
class Spider(CrawlSpider):
    ...
    def time_handler(self, str):
        mtime = self.settings['NOW']

        if str.find("今天") != -1:
            t = re.findall("[0-9]{2}", str)
            return mtime.replace(hour=int(t[0]), minute=int(t[1]))
        elif (str.find("月") != -1):
            t = re.findall("[0-9]{2}", str)
            return mtime.replace(month=int(t[0]), day=int(t[1]), hour=int(t[2]), minute=int(t[3]))
        elif (str.find("-") != -1):
            y = re.findall("[0-9]{4}", str)
            t = re.findall("[0-9]{2}", str[4:])
            return mtime.replace(year=int(y[0]), month=int(t[0]), day=int(t[1]), hour=int(t[2]), minute=int(t[3]),
                                 second=int(t[4]))
        return mtime
